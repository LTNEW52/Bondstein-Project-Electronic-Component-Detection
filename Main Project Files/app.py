...
def load_model():
    model = YOLO("Final_Project_Best_Model.pt")
    return model
...
# For video upload,
    ...
    if video_file:
        ...
        st.video(h264_path, autoplay=True) # Shows the processed video
...
# Lastly, Live video/webcam feed
    ...
    class YOLOTransformer(VideoTransformerBase):
        def __init__(self):
            self.frame_count = 0 # To track each frame
            self.counts = {}
        ...
        def transform(self, frame):
            ...
            if self.frame_count % 3 != 0: # To make it less laggy, we are taking every third image and run model on it
                return img
            ...
            # As usual, counts the component and draw bounding box and confidence
            self.counts = count_objects(results[0])
            annotated = results[0].plot()

            # Similar to video, we are writing counts on the live feed
            y_offset = 30
            line_height = 25
            if self.counts:
    ...
    webrtc_ctx = webrtc_streamer(
        key="example", # A unique ID, to find specific webcam, like giving a id to something to identify
        video_transformer_factory=YOLOTransformer,
        media_stream_constraints={"video": True, "audio": False},
        async_processing=True, # Else the frame will be frozed, so it takes the heavry transform process in the background
    )

    if webrtc_ctx.video_transformer:
        webrtc_ctx.video_transformer.conf = confidence # To update confidence using slider in real time